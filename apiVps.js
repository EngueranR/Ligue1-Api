...
async function getMatchData(page, journee) {
    try {
        ...
        const matchs = await page.$$eval(selectorJournee, (nodes, j) => {
            const scoreRegex = /\d+-\d+/; // scores sous la forme "n-n"
            return nodes.map((node, index) => {
                ...
                const score = node.querySelector('.result').textContent.trim();
                ...
                let score_domicile = null;
                let score_exterieur = null;
                if (score && !score.includes("21:00")) {
                    const scores = score.match(scoreRegex);
                    if (scores) {
                        [score_domicile, score_exterieur] = scores[0].split('-');
                    }
                }
                return {
                    id,
